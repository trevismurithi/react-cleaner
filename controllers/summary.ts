...
interface HydratedCodeGraph {
  graph: Graph;
  unusedFiles: Set<unknown>;
}

interface HydratedImageGraph {
  graph: ImageGraph;
  unusedImages: Set<unknown>;
}
...
interface TableSectionConfig {
  ...
  headers: string[];
  colWidths: number[];
  getRowData: (item: [string, AnyNode]) => string[];
  emptyMessageColor?: (text: string) => string;
}

export function formatFilePath(filePath: string, maxLength = 70): string {
...
function readCacheAndHydrateGraph(): {
  codeGraph: HydratedCodeGraph;
  imageGraph: HydratedImageGraph;
} {
...
export function summarizeAll(chalk: ChalkInstance): void {
  ...
  const formatValue = (value: number, isGood = false): string => {
    if (value === 0 && !isGood) {
...
function printSectionFooter(chalk: ChalkInstance): void {
  console.log(chalk.green("════════════════════════════════════════════════"));
}

function displayTableSection(
  chalk: ChalkInstance,
  data: Array<[string, AnyNode]> | null | undefined,
...
function displayHeavyDependencies(
  chalk: ChalkInstance,
  data: Array<[string, GraphNode]>
): void {
...
function displayLightDependencies(
  chalk: ChalkInstance,
  data: Array<[string, GraphNode]>
): void {
  displayTableSection(chalk, data, {
...
function displayFileHotspots(
  chalk: ChalkInstance,
  data: Array<[string, AnyNode]>
): void {
  displayTableSection(chalk, data, {
...
function displayDependencyHotspots(
  chalk: ChalkInstance,
  data: Array<[string, GraphNode]>
): void {
  displayTableSection(chalk, data, {
...
function displayReexports(
  ...
  displayTableSection(chalk, data, {
    ...
    getRowData: ([filePath, node]) => [
      chalk.white(formatFilePath(filePath, 85)),
      chalk.blue(((node as GraphNode).reExported?.size || 0).toString()),
    ],
  });
}

function displayReexportedBy(
  chalk: ChalkInstance,
  data: Array<[string, GraphNode]>
): void {
  displayTableSection(chalk, data, {
    ...
    getRowData: ([filePath, node]) => [
      chalk.white(formatFilePath(filePath, 85)),
      chalk.cyan(((node as GraphNode).reExportedBy?.size || 0).toString()),
    ],
  });
}

function displayDeadImageHotspots(
  chalk: ChalkInstance,
  data: Array<[string, AnyNode]>
): void {
  displayTableSection(chalk, data, {
...
function displayAliveImageHotspots(
  chalk: ChalkInstance,
  data: Array<[string, AnyNode]>
): void {
  displayTableSection(chalk, data, {
...
function collectDependencyData(
  ...
  const top10FilesHotspots = getTop10FilesDependenciesHotspots(
    codeGraph.graph,
    false
  );
  const top10FilesDependenciesHotspots = getTop10FilesDependenciesHotspots(
    codeGraph.graph
  );
  const top10FilesWithMostReexports = getTop10FilesWithMostReexports(
    codeGraph.graph
  );
  const top10FilesWithMostReexportedBy = getTop10FilesWithMostReexportedBy(
    codeGraph.graph
  );
...
export function dependenciesSummary(chalk: ChalkInstance): void {
  const { codeGraph, imageGraph } = readCacheAndHydrateGraph();
