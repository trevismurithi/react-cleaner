 ...
 * Report footer version is read from the installed package (e.g. 1.5.1).
 *
 * Usage: npx tsx .github/scripts/pr-report.ts <stats-file> <summary-file> [tidy-report-file] [image-report-file]
...
function resolveQleanerPackageRoot(): string {
  try {
    return path.dirname(nodeRequire.resolve("qleaner/package.json"));
  } catch {
    return path.join(__dirname, "../..");
  }
}

function loadFixPassKeys(): string[] {
  const pkgRoot = resolveQleanerPackageRoot();
  for (const rel of ["dist/utils/cache", "utils/cache"]) {
    try {
      return nodeRequire(path.join(pkgRoot, rel)).FIX_PASS_KEYS as string[];
    } catch {
      // try next candidate (published layout vs legacy)
    }
  }
...
const PKG_VERSION = (() => {
  try {
    return nodeRequire(path.join(resolveQleanerPackageRoot(), "package.json"))
      .version as string;
  } catch {
    return "unknown";
  }
...
let tidyReportPath: string;
let imageReportPath: string;
...
interface StatsFile {
  fileAssociatedStats?: Record<string, SessionStats>;
  statistics?: Record<string, TrashSession>;
}

function normalize(v: SessionStats): NormalizedStats {
  return {
    items: v.totalItemsRemoved ?? v.removedItems ?? v.removedCount ?? 0,
    lines: v.totalLinesRemoved ?? v.removedLines ?? v.locRemoved ?? 0,
    bytes: v.bytesSaved ?? v.removedBytes ?? (v.kbSaved ? parseFloat(String(v.kbSaved)) * 1024 : 0),
    files: v.filesModified ?? v.modifiedFiles ?? 0,
    hours: v.estimatedDeveloperHoursSaved ?? (v.timeSaved ? parseFloat(String(v.timeSaved)) : 0),
  };
}
...
/**
 * @param filePath
 * @param label Human label for placeholder text
 */
function ensureTextCaptureFile(filePath: string, label: string): void {
...
function ensureStatsFile(filePath: string): void {
  let needWrite = false;
  if (!fs.existsSync(filePath)) {
    needWrite = true;
...
const stats = JSON.parse(fs.readFileSync(statsPath, "utf8")) as StatsFile;
...
const byDay = new Map<string, { items: number; lines: number }>();
...
function buildCacheSnapshotSection(): string {
  if (!fs.existsSync(CACHE_FILE)) {
    return "_`unused-check-cache.json` not found after this run._";
  }
  try {
    const cache = JSON.parse(fs.readFileSync(CACHE_FILE, "utf8")) as {
      parentGraph?: {
      ...
      imageParentGraph?: {
        imageGraph?: Record<string, unknown>;
      ...
      for (const passKey of FIX_PASS_KEYS) {
        ...
        const fingerprintCount =
          bucket && typeof bucket === "object"
            ? Object.keys(bucket as Record<string, unknown>).length
            : 0;
...
function buildCliCaptureSection(
